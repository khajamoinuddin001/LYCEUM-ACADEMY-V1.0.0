...
import type { InterviewSession, QuestionWithScores, VisaType, Question, MockInterviewSession, QuestionTemplate, Contact } from '@/types';
import * as api from '@/utils/api';
import { MOCK_INTERVIEW_VISA_TYPES, DEFAULT_QUESTION_SCORES, calculateAverageScores, calculateOverallAverage, calculateVerdict } from '@/lib/constants';
...
const StepIndicator: React.FC<{ current: string; total: number }> = ({ current, total }) => {
    ...
    return (
        <div className="flex items-center justify-center mb-6 md:mb-8 overflow-x-auto no-scrollbar pb-2">
            <div className="flex items-center gap-2 md:gap-4 px-4 min-w-max">
                {steps.map((step, index) => (
                    <React.Fragment key={step}>
                        <div className="flex flex-col items-center">
                            <div
                                className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-bold text-sm md:text-base transition-all duration-300 ${index <= currentIndex
                            ...
                            <span className={`hidden sm:block mt-2 text-[10px] md:text-xs font-medium uppercase tracking-wider ${index <= currentIndex ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'
                                }`}>
                                {step}
                            </span>
                        </div>
                        {index < steps.length - 1 && (
                            <ChevronRight size={16} className={`md:w-5 md:h-5 ${index < currentIndex ? 'text-blue-500' : 'text-gray-300'
                                }`} />
                        )}
                    </React.Fragment>
                ))}
...
const NewSession: React.FC<NewSessionProps> = ({ contacts, onSave }) => {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                const [questions, tmpls] = await Promise.all([
                    api.getQuestions(),
                    api.getTemplates()
                ]);
    ...
    // Templates are now loaded in the first useEffect
...
Please provide a high-impact, professional feedback report using the following structure:

1. **EXECUTIVE SUMMARY** - A concise 1-paragraph overview of the student's performance and key verdict rationale.

2. **TOP 3 STRENGTHS**
   - Use bullet points to detail the most impressive aspects of the interview.
   - Explain briefly WHY these were effective for a visa interview.

3. **CRITICAL IMPROVEMENTS**
   - For any incorrect answers, provide a quick bulleted analysis:
     * Specific issue identified.
     * Brief explanation of why it might lead to a refusal.
     * The recommended "Best/Clear" answer.

4. **METRIC BREAKDOWN**
   - **Context (${averages.context}/5)**: High-impact bullet on understanding and relevance.
   - **Body Language (${averages.body_language}/5)**: Key observation on posture/eye contact/confidence.
   - **Fluency (${averages.fluency}/5)**: Key observation on speech flow and natural rhythm.
   - **Grammar & Vocabulary (${averages.grammar}/5)**: Key observation on structure and word choice.

5. **ACTIONABLE RECOMMENDATIONS**
   - Provide 3-5 specific, high-priority actions the student must take to improve.
   - Focus on "Best Practices" for a successful outcome.

6. **30-DAY SUCCESS ROADMAP**
   - A brief, bulleted weekly focus plan (Week 1 to Week 4).
   - High-impact practice routines.

Keep the report professional, extremely clear, and focused on "The Best" communication strategies. Use bullet points and bold formatting for maximum readability. Avoid unnecessary filler or lengthy paragraphs.`;

            const data = await api.generateAiFeedback(prompt);
...
${Object.entries(calculatedAverages)
                    .filter(([_, value]) => (value as number) < 3)
...
${Object.entries(calculatedAverages)
                    .map(([key, value]) => {
                        if ((value as number) < 3) return `- Practice ${key.replace('_', ' ')} to improve from ${value}/5`;
                        if ((value as number) < 4) return `- Continue working on ${key.replace('_', ' ')} to reach 4+/5`;
                        return '';
                    })
                    .filter(Boolean)
        ...
        <div className="space-y-6">
            ...
            {step === 'student' && (
                <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-3xl shadow-xl p-6 md:p-8 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Select Student</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div className="relative">
                                <div
                                    ...
                                    className={`w-full px-4 py-3 border-2 rounded-xl cursor-pointer flex items-center justify-between transition-all ${selectedContact
                                ...
                                >
                                    ...
                                    <ChevronDown
                                        size={18}
                                        className={`text-gray-500 dark:text-gray-400 transition-transform ${showStudentDropdown ? 'rotate-180' : ''
                                            }`}
                                    />
                                </div>
                                            ...
                                            <div className="py-2">
                                                {filteredStudents.map(contact => (
                                                    <div
                                                        ...
                                                        className={`px-4 py-3 hover:bg-purple-50 dark:hover:bg-purple-900/20 cursor-pointer transition-colors ${selectedContact?.id === contact.id
                        ...
                        <div>
                            ...
                            <select
                                ...
                                className="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-100 transition-all text-sm md:text-base"
                                required
                            >
            ...
            {step === 'questions' && (
                <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-3xl shadow-xl p-6 md:p-8 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Select Questions</h2>
                    </div>
                    ...
                    <div className="flex gap-2 mb-6 p-1 bg-gray-100 dark:bg-gray-700 rounded-xl">
                        <button
                            ...
                            className={`flex-1 px-4 py-2.5 rounded-lg font-medium transition-all ${questionMode === 'individual'
                        ...
                        <button
                            ...
                            className={`flex-1 px-4 py-2.5 rounded-lg font-medium transition-all ${questionMode === 'template'
                                    ? 'bg-white dark:bg-gray-600 text-purple-600 dark:text-purple-400 shadow-md'
                                    : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
                                }`}
                        >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto pr-2">
                                    {templates.map(template => (
                                        <div
                                            ...
                                            className={`p-5 rounded-2xl cursor-pointer transition-all duration-300 border-2 ${selectedTemplate?.id === template.id
                                        ...
                                        >
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold ${template.difficulty === 'Easy' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' :
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto pr-2">
                                {availableQuestions.map(question => (
                                    <div
                                        ...
                                        className={`p-5 rounded-2xl cursor-pointer transition-all duration-300 border-2 ${selectedQuestions.find(q => q.id === question.id)
                                                ? 'border-purple-500 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-purple-900/20 dark:to-pink-900/20 shadow-lg'
                                                : 'border-gray-200 dark:border-gray-600 hover:border-purple-300 hover:shadow-lg dark:hover:border-purple-700'
                                            }`}
                                    >
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            {question.difficulty && (
                                                <span className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold ${question.difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' :
            ...
            {step === 'marking' && (
                <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-3xl shadow-xl p-6 md:p-8 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Mark Question Responses</h2>
                    ...
                    <div className="space-y-4">
                        {selectedQuestions.map((question, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={`p-6 rounded-2xl border-2 transition-all ${isMarked
                                    ...
                                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <button
                                                ...
                                                className={`flex items-center justify-center py-4 px-6 rounded-xl border-2 font-bold transition-all duration-300 ${response?.is_correct === true
                                            ...
                                            <button
                                                ...
                                                className={`flex items-center justify-center py-4 px-6 rounded-xl border-2 font-bold transition-all duration-300 ${response?.is_correct === false
            ...
            {step === 'scoring' && (
                <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-3xl shadow-xl p-6 md:p-8 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Score Overall Performance</h2>
                    </div>
            ...
            {step === 'verdict' && (
                <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-3xl shadow-xl p-6 md:p-8 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Review & Submit Session</h2>
                    </div>

                    {calculatedVerdict && (
                        <div className="mb-8">
                            <div className={`p-6 rounded-2xl text-center shadow-lg ${(finalVerdict || calculatedVerdict.verdict) === 'Approved' ?
                    ...
                    <div className="flex flex-col sm:flex-row justify-between gap-4 mt-8">
                        ...
                        <button
                            ...
                            className={`flex items-center px-10 py-4 rounded-2xl font-bold text-lg transition-all duration-300 hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 ${calculatedVerdict?.verdict === 'Review Required' && !finalVerdict
            ...
            {showApprovalModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-3xl shadow-2xl p-6 md:p-8 w-full max-w-lg border border-gray-100 dark:border-gray-700">

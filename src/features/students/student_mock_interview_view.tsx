...
const StudentMockInterviewView: React.FC<StudentMockInterviewViewProps> = ({ user, sessions = [] }) => {
    ...
    return (
        <>
            ...
            {selectedSession && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-0 md:p-4 print:p-0 print:static print:block print:bg-white">
                    <div className="bg-white dark:bg-gray-800 rounded-none md:rounded-[2.5rem] w-full max-w-4xl h-full md:h-auto md:max-h-[90vh] shadow-2xl overflow-hidden flex flex-col animate-fade-in border border-white/10 print:h-auto print:max-h-none print:shadow-none print:border-none print:rounded-none print:block print:static">
                        <div className="bg-gradient-to-r from-rose-500 to-rose-600 p-6 md:p-8 text-white flex justify-between items-center print:bg-none print:text-black print:border-b-2 print:border-rose-500 print:mb-8">
                            <div>
                                <h1 className="text-xl md:text-3xl font-black print:text-3xl">{selectedSession.visa_type} Interview Report</h1>
                                <p className="text-rose-100 text-sm md:font-medium print:text-gray-600 font-bold">{new Date(selectedSession.session_date).toLocaleDateString()}</p>
                            </div>
                        ...
                        <div className="p-4 md:p-8 overflow-y-auto space-y-6 md:space-y-8 print:p-0 print:overflow-visible print:block print:h-auto">
                            {/* Performance Grid */}
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 print:gap-4 print:grid-cols-4">
                                <div className="bg-rose-50 dark:bg-rose-900/20 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-rose-100 dark:border-rose-800 print:bg-white print:border-gray-200">
                                    <div className="text-rose-600 dark:text-rose-400 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1 print:text-gray-500">Overall Score</div>
                                    <div className="text-2xl md:text-4xl font-black text-rose-700 dark:text-rose-300 print:text-black">{selectedSession.overall_average.toFixed(1)}</div>
                                </div>
                                <div className="bg-gray-50 dark:bg-gray-900 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-800 print:bg-white print:border-gray-200">
                                    <div className="text-gray-500 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1">Accuracy</div>
                                    <div className="text-2xl md:text-4xl font-black text-gray-900 dark:text-white print:text-black">
                                        {selectedSession.questions_count > 0 
                                            ? ((selectedSession.correct_count / selectedSession.questions_count) * 100).toFixed(0) 
                                            : 0}%
                                    </div>
                                </div>
                                <div className="bg-gray-50 dark:bg-gray-900 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-800 font-black flex flex-col justify-center print:bg-white print:border-gray-200">
                                    ...
                                    } print:text-black font-black`}>
                                        {selectedSession.verdict}
                                    </div>
                                </div>
                                <div className="bg-gray-50 dark:bg-gray-900 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-800 print:bg-white print:border-gray-200">
                                    <div className="text-gray-500 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1">Total Questions</div>
                                    <div className="text-2xl md:text-4xl font-black text-gray-900 dark:text-white print:text-black">{selectedSession.questions_count}</div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 print:grid-cols-1 print:gap-6">
                                <div className="space-y-6">
                                    <h3 className="text-xl font-black flex items-center gap-3 text-gray-900 dark:text-white print:text-black">
                                        <TrendingUp size={24} className="text-rose-500 print:hidden" /> Performance Breakdown
                                    </h3>
                                    <div className="space-y-5 bg-gray-50 dark:bg-gray-900/50 p-8 rounded-3xl print:bg-white print:p-0 print:border-none">
                                        {[
                                            ...
                                            <div key={score.label} className="space-y-2">
                                                <div className="flex justify-between text-sm font-bold">
                                                    <span className="text-gray-600 dark:text-gray-400 print:text-gray-700">{score.label}</span>
                                                    <span className="text-gray-900 dark:text-white print:text-black">{score.value.toFixed(1)}/5.0</span>
                                                </div>
                                                <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden print:bg-gray-100 print:border print:border-gray-300">
                                                    <div className="h-full bg-rose-500 rounded-full print:bg-gray-800" style={{ width: `${(score.value / 5) * 100}%` }} />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="space-y-6">
                                    <h3 className="text-xl font-black flex items-center gap-3 text-gray-900 dark:text-white print:text-black">
                                        <AlertCircle size={24} className="text-rose-500 print:hidden" /> Expert Feedback
                                    </h3>
                                    <div className="bg-rose-50/50 dark:bg-rose-900/10 p-8 rounded-3xl border-2 border-rose-100 dark:border-rose-900/30 text-rose-900 dark:text-rose-100 leading-relaxed font-medium whitespace-pre-wrap print:bg-white print:border-none print:p-0 print:text-black">
                            ...
                            <div className="space-y-6 print:mt-12">
                                <h3 className="text-xl font-black flex items-center gap-3 text-gray-900 dark:text-white print:text-black">
                                    <BookOpen size={24} className="text-rose-500 print:hidden" /> Interview Question Log
                                </h3>
                                <div className="grid grid-cols-1 gap-4 print:gap-2">
                                    {selectedSession.questions.map((q, i) => (
                                        <div key={i} className="flex flex-col sm:flex-row gap-4 md:gap-6 p-4 md:p-6 rounded-3xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm print:border-none print:p-2 print:flex-row">
                                            <div className="flex items-center gap-3 sm:block sm:mt-1">
                                                {q.is_correct ? <CheckCircle size={24} className="text-green-500 print:text-black" /> : <X size={24} className="text-red-500 print:text-black" />}
                                                <span className="sm:hidden font-bold text-gray-900 dark:text-white print:inline">Q{i + 1}</span>
                                            </div>
                                            <div className="flex-1 space-y-3">
                                                <div className="font-black text-gray-900 dark:text-white text-base md:text-lg print:text-black print:text-sm">Q{i + 1}: {q.question_text}</div>
                                                {q.notes && (
                                                    <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-2xl text-sm text-gray-600 dark:text-gray-400 font-medium print:bg-white print:p-0 print:text-xs">
                                                        <span className="text-rose-500 font-black uppercase text-[10px] block mb-1 print:text-gray-500">Counsellor Notes</span>
                                                        {q.notes}
                                                    </div>
                                                )}
                                            </div>

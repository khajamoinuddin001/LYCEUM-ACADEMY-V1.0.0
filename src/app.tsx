...
import AppsGridView from '@/features/shared/apps_grid_view';
import Dashboard from '@/features/dashboard/dashboard';
import DiscussView from '@/features/chat/discuss_view';
import AppView from '@/features/shared/app_view';
import TasksView from '@/features/tasks/tasks_view';
import TaskModal from '@/features/tasks/task_modal';
import ContactsView from '@/features/students/contacts_view';
import ProfileView from '@/features/settings/profile_view';
import StudentProfileView from '@/features/students/student_profile_view';
import SettingsView from '@/features/settings/settings_view';
import CalendarView from '@/features/calendar/calendar_view';
import CrmView from '@/features/crm/crm_view';
import AccountingView from '@/features/finance/accounting_view';
import ReceptionView from '@/features/reception/reception_view';
...
import ContactVisaView from '@/features/students/contact_visa_view';
import LeadDetailsModal from '@/features/crm/lead_details_modal';
import NewLeadModal from '@/features/crm/new_lead_modal';
import TransactionModal from '@/features/finance/transaction_modal';
import PrintTransaction from '@/features/finance/print_transaction';
...
import AIEmailComposerModal from '@/features/shared/aiemail_composer_modal';
import ContactChecklistView from '@/features/students/contact_checklist_view';
import NewVisitorModal from '@/features/reception/new_visitor_modal';
import NewAppointmentModal from '@/features/reception/new_appointment_modal';
...
import ResetPasswordForm from '@/features/auth/reset_password_form';
import LmsView from '@/features/lms/lms_view';
import CourseDetailView from '@/features/lms/course_detail_view';
...
import EventModal from '@/features/calendar/event_modal';
import LandingPage from '@/features/marketing/landing_page';
import TicketsView from '@/features/support/tickets_view';
import StudentTicketsView from '@/features/students/student_tickets_view';
import StudentAppsView from '@/features/students/student_apps_view';
...
import VerifyEmail from '@/features/auth/verify_email';
import DuolingoPage from '@/features/lms/courses/DuolingoPage';
import IELTSPage from '@/features/lms/courses/IELTSPage';
import PTEPage from '@/features/lms/courses/PTEPage';
import TOEFLPage from '@/features/lms/courses/TOEFLPage';
import TermsPage from '@/features/marketing/footer/TermsPage';
import PrivacyPage from '@/features/marketing/footer/PrivacyPage';
  ...
  const handleSaveTransaction = async (data: Omit<AccountingTransaction, 'id'> & { id?: string }) => {
    try {
      if (data.id) {
        ...
        // Handle Linking to AR entry - Supporting Pending and Paid
        if ((data as any).linkedArId && data.contactId) {
          const contact = contacts.find(c => c.id === data.contactId);
          if (contact) {
            const updatedMetadata = { ...((contact as any).metadata || {}) };
            let arUpdated = false;
            let linkedLeadId = data.linkedLeadId || tx.linkedLeadId;
            let linkedQuotationId = data.linkedQuotationId || tx.linkedQuotationId;
              ...
              if (ar.id === (data as any).linkedArId) {
                ...
                const updatedArLineItems = (ar.lineItems || []).map((arItem: any) => {
                  ...
                  if (wasLinked && oldTx && oldTx.lineItems) {
                    ...
                    if (oldItem) {
                      oldInvoiceItemInvoiced = oldItem.amount || 0;
                      oldInvoiceItemPaid = (oldTx.status === 'Paid') ? (oldItem as any).received || 0 : 0;
                    }
                  }
                  ...
                  if (linkedInvoiceItem) {
                    ...
                    return {
                      ...arItem,
                      invoicedAmount: Math.max(0, (Number(arItem.invoicedAmount) || Number(arItem.paidAmount) || 0) + itemInvoicedDiff),
                      paidAmount: Math.max(0, (Number(arItem.paidAmount) || 0) + itemPaidDiff)
                    };
                  }
                  return arItem;
                });
                ...
                arUpdated = true;
                // Capture lead/quotation IDs from AR if not in transaction data
                if (!linkedLeadId) linkedLeadId = ar.linkedLeadId;
                if (!linkedQuotationId) linkedQuotationId = ar.linkedQuotationId;
                ...
                return {
                  ...
                  status: newPaidAmount >= totalCost ? 'Paid' : (newInvoicedAmount > 0 ? 'Partial' : 'Pending'),
            ...
            if (arUpdated) {
              ...
              if (linkedLeadId && linkedQuotationId) {
                try {
                  const lead = await api.getLead(linkedLeadId);
                  if (lead && lead.quotations) {
                    const arEntry = updatedMetadata.accountsReceivable.find((ar: any) => ar.id === (data as any).linkedArId);
                    const updatedQuotations = lead.quotations.map((q: any) => {
                      if (String(q.id) === String(linkedQuotationId)) {
                        const subtotal = arEntry.lineItems.reduce((sum: number, item: any) => sum + (Number(item.price) * (Number(item.quantity) || 1)), 0);
                        const discount = Number(arEntry.additionalDiscount) || 0;
                        return {
                          ...q,
                          lineItems: arEntry.lineItems,
                          discount: discount,
                          subtotal: subtotal,
                          total: Math.max(0, subtotal - discount)
                        };
        ...
        // Handle Linking to AR entry if specified - Supporting Pending and Paid
        if ((data as any).linkedArId && data.contactId) {
          const contact = contacts.find(c => c.id === data.contactId);
          if (contact) {
            const updatedMetadata = { ...((contact as any).metadata || {}) };
            ...
            updatedMetadata.accountsReceivable = (updatedMetadata.accountsReceivable || []).map((ar: any) => {
              if (ar.id === (data as any).linkedArId) {
                ...
                const updatedArLineItems = (ar.lineItems || []).map((arItem: any) => {
                  const linkedInvoiceItem = (data.lineItems || []).find((invItem: any) => invItem.linkedQuotationLineItemId === arItem.id);
                  if (linkedInvoiceItem) {
                    const itemInvoiced = Number(linkedInvoiceItem.amount) || 0;
                    const itemPaid = Number(linkedInvoiceItem.received) || 0;
                    return {
                      ...arItem,
                      invoicedAmount: (Number(arItem.invoicedAmount) || 0) + itemInvoiced,
                      paidAmount: (Number(arItem.paidAmount) || 0) + itemPaid
                    };
                  }
                  return arItem;
                });
        ...
        if (arIndex !== -1) {
          ...
          try {
            ...
            if (txToDelete.linkedLeadId && txToDelete.linkedQuotationId) {
              ...
              if (lead && lead.quotations) {
                const updatedQuotations = lead.quotations.map((q: any) => {
                  if (String(q.id) === String(txToDelete.linkedQuotationId)) {
                    const subtotal = arEntry.lineItems.reduce((sum: number, item: any) => sum + (Number(item.price) * (Number(item.quantity) || 1)), 0);
                    const discount = Number(arEntry.additionalDiscount) || 0;
                    return {
                      ...q,
                ...
                await api.saveLead({ ...lead, quotations: updatedQuotations }, false);
              }
            }
          } catch (e) {
            console.error("Failed to reverse AR tracking during deletion", e);
          }
        }
      }
    }
